...
import { tagBookmark } from '../services/tagger.js';
...
app.post('/', async (c) => {
  ...
  const bookmark = rowToBookmark(result.rows[0] as Record<string, unknown>);

  // AIタグ付けを非同期で実行（レスポンスをブロックしない）
  tagBookmark(id, body.url ?? null, body.title ?? null, body.memo ?? null);
